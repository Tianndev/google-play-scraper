...
try:
    ...
    print(">> Sedang Menginstall Module google_play_scraper <<")
...
{WHITE} [ Script For {RED}Google Play{WHITE} Scraper | Developed By {LIGHT_ORANGE}Tiann Dev[bold blue]𓄀[/] {WHITE}]"""
...
def main():
    while True:
        ...
        if choice == '1':
            ...
            prints(panel(f"{RED}Masukkan package id aplikasi yang akan Anda lihat & simpan ulasannya\nContoh Package ID: {GREEN}com.instagram.android", title="[yellow]Peringatan!", style="bold white"))
            app_id = input("Masukkan ID aplikasi Google Play Store: ")
            ...
            with console.status("[bold cyan]Mengumpulkan ulasan...") as status:
                while total_reviews < 1000:
                    ...
                    console.print(f"\r{rc(frames)} Mengumpulkan ulasan...", end="")
                    time.sleep(1)
            prints(panel(f"{RED}[!] {WHITE}Jumlah ulasan yang berhasil dikumpulkan: {GREEN}{total_reviews}", style="bold white"));time.sleep(3)
            idx = 0
            ...
            if input(" Pilih: (y/n): ").strip().lower() == 'y':
                filename = print_reviews_to_csv(all_reviews, app_id)
